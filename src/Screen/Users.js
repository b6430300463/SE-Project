...
const Users = () => {
  ...
  const updatePriority = async () => {
    for (const email in priority) {
      if (priority.hasOwnProperty(email)) {
        ...
        try {
          const response = await axios.post(`${url}/api/updatePriority`, {
            email,
  ...
  const addEmailAndPriority = async (email, priority) => {
    ...
    try {
      const response = await axios.post(`${url}/api/manual_insertUser`, {
        data: userData,
      });
  ...
  const deleteUser = (email) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${url}/api/deleteusers`, { data: { email } })
          .then((response) => {
            if (response.data.message) {
              Swal.fire({
                title: "ลบแล้ว!",
  ...
  return (
    <div className="input-container">
      ...
      <div className="sub-box">
        ...
        />
        <label style={{ paddingLeft: 20, fontWeight: "bold" }}>
          Priority :{" "}
        </label>
        <select
          name="major-id"
        ...
        <button
          ...
          id="submit-input"
          onClick={() => {
            addEmailAndPriority(newEmail, newPriority);
          }}
        >
          <strong>ADD</strong>
        </button>
